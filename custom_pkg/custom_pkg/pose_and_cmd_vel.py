...
from custom_interfaces.msg import CustomMessag  # 사용자 정의 메시지 CustomMessag을 사용하기 위해 불러옵니다.
...
class CmdAndPose(Node):
    ...
    def __init__(self):
        ...
        self.pub_custom_msg = self.create_publisher(CustomMessag, '/cmd_and_pose', 10)
        
        # 타이머를 생성하고, timer_callback 메서드를 주기적으로 호출합니다.
        self.timer = self.create_timer(self.timer_period, self.timer_callback)
        
        # 사용자 정의 메시지와 Twist 메시지를 초기화합니다.
        self.cmd_pose = CustomMessag()
        self.cmd_vel = Twist()
    ...
    def callback_pose(self, msg):
        ...
        self.get_logger().info("x: %f, y: %f" % (self.cmd_pose.x, self.cmd_pose.y))
        self.get_logger().info("linear velocity: %f, angular velocity: %f" % (self.cmd_pose.cmd_linear_vel, self.cmd_pose.cmd_angular_vel))
